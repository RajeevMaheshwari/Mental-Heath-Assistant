...
def prep_rag_prompt(
    ...
    # Prepares the prompt for mental health-related queries, adding empathetic tone
    docs = docs.value
    try:
        docs_str = "\n\n".join([doc["text"] for doc in docs])
    except Exception:
        docs_str = "No relevant documents were found."
    
    if response_type == AIResponseType.SHORT.value:
        # Short, empathetic response
        return f"Please, with care and kindness, answer the following question based on the information: \n\n {docs_str} \n\n Query: {prompt}"
    else:
        # Long, detailed, and supportive response
        return f"Taking into account the emotional well-being of the user, provide a detailed response to the following based on the information provided: \n\n {docs_str} \n\n Query: {prompt}"
...
def prompt_aggregate(question: str, answers: list[str]) -> str:
    ...
    prompt = f"""You are providing a consolidated response to a mental health query.
    Based on the following question: "{question}" and the following multiple answers:
    ```{summaries_str}```
    Please carefully and empathetically aggregate the answers.
    Ensure that your response promotes emotional well-being, avoids negative triggers, and is supportive.
